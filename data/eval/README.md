# RAG_researcher Evaluation Framework

This workspace is the source of truth for resume-grade RAG_researcher evaluation. Keep every number reproducible from the files in this directory and reports under `reports/evaluation`.
...
```cmd
uv run rag_researcher-eval run --dataset data/eval/retrieval.jsonl --strategies bm25,dense,hybrid,hybrid_hyde,hybrid_hyde_rerank --top-k 1,3,5,10
```

Run the full benchmark plan:

```cmd
uv run rag_researcher-eval benchmark --dataset data/eval/qa_gold.json --plan data/eval/ablation_plan.json --judge data/eval/answer_judge.json --top-k 1,3,5,10 --output reports/evaluation/benchmark.json
```
...
- Build one baseline index without visual/fused chunks: `rag_researcher_body_only_chunks`.
- Build one enhanced index with body/visual/fused chunks: `rag_researcher_chunks`.
