...
from rag_researcher.chunking.block_chunker import chunk_block_documents, map_documents_by_arxiv_id
from rag_researcher.chunking.fixed_chunker import Chunk, estimate_token_count
from rag_researcher.chunking.repository import DocumentRecord
from rag_researcher.config import settings
from rag_researcher.ingestion.hashing import content_hash
from rag_researcher.papers.metadata_noise import is_metadata_noise as is_shared_metadata_noise
from rag_researcher.papers.models import PaperBlock
from rag_researcher.chunking.section_names import resolve_section_name
from rag_researcher.utils.text import sanitize_text
...
class DeepSeekChunkPlanner:
    def __init__(self, api_key: str | None, model: str) -> None:
        from rag_researcher.generation.generator import DeepSeekGenerator
        self.generator = DeepSeekGenerator(api_key=api_key, model=model)

    def is_configured(self) -> bool:
        return self.generator.is_configured()

    def generate(self, prompt: str, system_prompt: str = AGENTIC_CHUNK_SYSTEM_PROMPT) -> PlannerGenerationResult:
        result = self.generator.generate(prompt, system_prompt=system_prompt, json_mode=True)
        return PlannerGenerationResult(
...
def build_planner(provider: str | None, model: str | None) -> ChunkPlanningModel:
    ...
    if selected_provider == "deepseek":
        return DeepSeekChunkPlanner(settings.deepseek_api_key, selected_model)
