...
from rag_researcher.runtime import get_ollama_decision_generator
...
def evaluate_guardrail(query: str) -> GuardrailDecision:
    ...
    alnum_chars = re.findall(r"[^\W_]", text)
    ...
    from rag_researcher.agent.policies.intent import is_direct_answer_query, is_local_knowledge_query
    if is_direct_answer_query(text) or is_local_knowledge_query(text):
        return GuardrailDecision(True, "intent_match_passed")
