...
from rag_researcher.agent.policies.intent import is_current_or_external_query
...
def grade_documents(
    ...
    if settings.agent_decision_enabled:
        ...
        if decision is not None:
            return decision
        # LLM call failed (network error, timeout, etc.) — trust retrieval
        return GradeDecision(
            relevant=True,
            sufficient=True,
            score=0.8,
            reason="agent_decision_unavailable_accept",
            useful_chunk_ids=list(range(1, len(documents) + 1)),
        )
