...
class OllamaEncoder:
    def __init__(
        ...
        print(f"initialized OllamaEncoder: model={model_name} dimension={expected_dimension} base_url={self.base_url}")
    ...
    def encode(self, texts: list[str], batch_size: int = 16) -> list[list[float]]:
        ...
        print(
            "before encode: "
            f"text_count={len(texts)} batch_size={batch_size} provider=ollama model={self.model_name}"
        )
        ...
        print(f"after encode: text_count={len(texts)} provider=ollama")

        for vector in all_embeddings:
            if len(vector) != self.expected_dimension:
                raise ValueError(
                    f"Expected {self.expected_dimension} dimensions from {self.model_name}, got {len(vector)}"
                )
