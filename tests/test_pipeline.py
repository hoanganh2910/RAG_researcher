...
from rag_researcher.agent.policies.grader import grade_documents
from rag_researcher.agent.policies.rewriter import clean_rewritten_query, rewrite_query
from rag_researcher.agent.policies.router import route_query
from rag_researcher.chunking.fixed_chunker import chunk_document
from rag_researcher.chunking.repository import (
...
from rag_researcher.embedding.repository import (
...
from rag_researcher.ingestion.repository import (
    count_documents_by_hash,
    ensure_documents_table,
    insert_documents,
)
from rag_researcher.papers.models import PaperBlock, PaperMetadata
from rag_researcher.papers.repository import (
...
from rag_researcher.retrieval.indexer import fetch_embedded_chunks, index_embedded_chunks
from rag_researcher.retrieval.search import bm25_search, hybrid_search, vector_search
...
def check_fastapi_app() -> bool:
    return app.title == "RAG_researcher"
...
def check_capability_performance_optimization() -> dict[str, bool]:
    encoder_a = get_embedding_encoder(settings.embedding_model)
    encoder_b = get_embedding_encoder(settings.embedding_model)
    ...
    generator_a = get_answer_generator()
    generator_b = get_answer_generator()
...
def check_capability_architecture_visualization() -> dict[str, bool]:
    ...
    workflow_diagram = PROJECT_ROOT / "docs" / "images" / "rag_researcher-langgraph-workflow.drawio"
    system_diagram = PROJECT_ROOT / "docs" / "images" / "rag_researcher-architecture.drawio"
    workflow_image = PROJECT_ROOT / "docs" / "images" / "rag_researcher-langgraph-workflow.svg"
    system_image = PROJECT_ROOT / "docs" / "images" / "rag_researcher-architecture.svg"
    workflow_png = PROJECT_ROOT / "docs" / "images" / "rag_researcher-langgraph-workflow.png"
    system_png = PROJECT_ROOT / "docs" / "images" / "rag_researcher-architecture.png"
...
def main() -> None:
    ...
    print("RAG_researcher production smoke check")
    print("=================================")
