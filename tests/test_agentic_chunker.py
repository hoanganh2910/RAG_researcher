...
from rag_researcher.chunking.fixed_chunker import Chunk
...
def test_deepseek_is_not_supported_as_chunk_planner() -> None:
    # deepseek is now a supported chunk planner provider
    from rag_researcher.chunking.agentic_chunker import DeepSeekChunkPlanner
    planner = build_planner("deepseek", "deepseek-chat")
    assert isinstance(planner, DeepSeekChunkPlanner)


def test_unknown_provider_raises_for_chunk_planner() -> None:
    with pytest.raises(ValueError, match="Unsupported chunk planner provider"):
        build_planner("unknown_provider", "some-model")
...
def table_confidence_for_test(content: str) -> str:
    from rag_researcher.chunking.agentic_chunker import table_confidence
