...
from rag_researcher.pipeline.runner import PipelineConfig, run_knowledge_pipeline, run_with_retry
...
async def test_pipeline_summary_tracks_steps_and_skips(monkeypatch) -> None:
    ...
    monkeypatch.setattr("rag_researcher.pipeline.runner.fetch_and_store_papers", fake_fetch)
    monkeypatch.setattr("rag_researcher.pipeline.runner.chunk_documents", fake_chunk)
    monkeypatch.setattr("rag_researcher.pipeline.runner.embed_chunks", fake_embed)
    monkeypatch.setattr("rag_researcher.pipeline.runner.index_embeddings", fake_index)
