...
RAG_researcher 是一个本地优先的 Agentic RAG 检索问答系统，面向论文、企业知识库、技术手册等复杂长 PDF。项目重点解决 vanilla RAG 在复杂问题、长上下文召回、图表/表格证据、查询改写、重排和引用级答案溯源上不稳定的问题。
...
![RAG_researcher 系统架构](docs/images/rag_researcher-system-architecture.png)

RAG_researcher 分为离线知识库构建链路和在线 Agentic RAG 问答链路。

离线链路负责把原始 PDF 转成可检索证据：

```text
arXiv / 上传 PDF / 企业文档
 -> 获取元数据并下载 PDF
 -> 使用 Docling / PyMuPDF 解析 PDF
 -> 存储结构化 paper_blocks
 -> 使用 qwen3.5:4b 规划 agentic chunks
 -> 构建 body / visual / fused chunks
 -> 边界校验与图表证据关联
 -> 使用 Qwen3-Embedding-4B 生成 chunk 向量
 -> 元数据写入 PostgreSQL，检索索引写入 Elasticsearch
```

在线链路通过 FastAPI 和 LangGraph 完成检索问答：

```text
Client / CLI / HTTP
 -> FastAPI endpoints
 -> LangGraph guardrail 与 router
 -> direct answer 或 local RAG
 -> 可选 HyDE
 ...
 -> generate answer 或 rewrite query 或 give up
 -> answer with sources / 可选 SSE stream
```

PostgreSQL 保存 papers、paper_files、paper_blocks、document_chunks 和 chunk_embeddings。Elasticsearch 提供 BM25 与向量检索。Redis 用于 QA/runtime cache 和 feedback buffer。Langfuse traces、trace summary、retrieval metadata、rerank status 与 pytest regression tests 支撑可观测性和回归评测。

### Agent 工作流

![RAG_researcher LangGraph 工作流](docs/images/rag_researcher-agent-workflow.png)
...
![RAG_researcher 评测总览](docs/images/rag_researcher-evaluation-summary.png)
...
src/rag_researcher/agent/           LangGraph 工作流与 agent 策略
src/rag_researcher/chunking/        固定切片、block 切片、agentic 切片
src/rag_researcher/cli/             解析、索引、检索、评测命令行工具
src/rag_researcher/evaluation/      评测框架与检索指标
src/rag_researcher/embedding/       embedding 编码与存储
src/rag_researcher/papers/          arXiv 下载与 PDF 解析
src/rag_researcher/retrieval/       Elasticsearch BM25/向量检索与 RRF
src/rag_researcher/reranking/       BGE reranker 封装
